module parameters
    ...
    integer,parameter  :: order           = 3  ! 1=MIRK_1st;2=MIRK_2nd or SSP2(222);3=SSP2-LUM;4=SSP3(433);5=BPR(353);6=MEX-RK36S_E;8=IMEX-RK48SSP
    integer,parameter  :: MIRK            = 0  ! use 1 for MIRK_1st 2 for MIRK_2nd and 0 for IMEX
    integer,parameter  :: DIM             = 2  ! use 1 for 1D,  2 for 2D and 3 for 3D
    integer,parameter  :: fix             = 0  ! use 1 for fix loop counter in 1D tests and 0 for 2D and 3D tests
    integer,parameter  :: TEST            = 25 ! look down the number assigned for each test
    ...
    integer,parameter  :: slop            = 2  ! use 1 for MinMod 2 for MCL and 3 for Superbee (in flux reconstruction)
    integer,parameter  :: limit           = 2  ! use 1 for MinMod 2 for MCL and 3 for Superbee (in primitive variables reconstruction )

    ! Recovery and reconstruction options
    integer,parameter  :: flux_solver     = 2 ! use 1 for Lax-Friederich flux, 2 for HLL + MCL, 3 for HLLE + MCL, 4 for HLLC + MCL , 5 for HLL + MP5, 6 for HLLE + MP5 and 7 HLLC + MP5
    ...
    integer,parameter  :: flux_rec_mp5    = 0  ! use 1 for reconstruction of fluxes using mp9 or mp5 and 0 for explicit formulation of fluxes using reconstruc mp9 or mp5 conserved variables
...
!____________________________________
  ...
  !     NewRaph ---> nr
    ...
    integer, parameter :: hmax  = 601
    integer, parameter :: imax  = 200
    integer, parameter :: jmax  = 200  ! 1 for 1D 
    ...
    integer, parameter :: kmed = floor(0.5d0*kmax)
    integer            :: ierror
    ...
    doubleprecision,parameter :: B0        =  1.d-1
    doubleprecision,parameter :: V0        =  0.d0 
    doubleprecision,parameter :: P_up      =  1.d0
    doubleprecision,parameter :: P0        =  1.d0
    doubleprecision,parameter :: alpha_rec =  0.5d0 * pi
    doubleprecision,parameter :: alpha_kh  =  0.1d0
    doubleprecision,parameter :: a_kh      =  0.025d0
    doubleprecision,parameter :: vsh       =  0.5d0
    ...
    doubleprecision,parameter :: VA_rec         =  sqrt(sigma_m/(1.d0+ sigma_m))   !Zenitani et al. (2010) (used in reconnection)                                                                   
    ...
    doubleprecision,parameter ::  S_l      =  1.d3 

    !   Amplitud TM perturbation

    doubleprecision,parameter :: epsilon_tm=  1.d-3


    !   Simulation domine

    doubleprecision,parameter ::  Lr       =  3.d0
    doubleprecision,parameter ::  a_tm     =  S_l**(-1.d0/3.d0) * Lr ! OJO CAMBIAR EXPONENTE PARA REC, en especial (interesante) usar Sl**(-1./3.)
    doubleprecision,parameter ::  l_tm     =  5.d0 * a_tm  !for DTM
    doubleprecision,parameter ::  k_tm     =  12.d0        !for TM
    doubleprecision,parameter ::  k_kh     =  2.d0 * pi
    doubleprecision,parameter ::  kap_tm   =  k_tm * a_tm  !for DTM
    doubleprecision,parameter ::  Lx       =  1.d0  
    doubleprecision,parameter ::  Ly       =  1.d0
    doubleprecision,parameter ::  Lz       =  1.d0  
    doubleprecision,parameter ::  k_x      =  2.d0  * pi / Lx 
    doubleprecision,parameter ::  k_y      =  2.d0  * pi / Ly
    doubleprecision,parameter ::  k_z      =  2.d0  * pi / Lz
    ...
    doubleprecision,parameter :: sigma_0   =  1.d6
    ...
    doubleprecision           :: r, posx, posy, posz,x, omega
    ...
    doubleprecision,parameter                   :: Longz = Lz
    doubleprecision,parameter                   :: Delx  = Longx  / real(imax)
    doubleprecision,parameter                   :: Dely  = Longy  / real(jmax)
    doubleprecision,parameter                   :: Delz  = Longz  / real(kmax)
    doubleprecision,parameter                   :: CFL   = 0.1d0

    doubleprecision                             :: t_advance
    doubleprecision                             :: Delt  = CFL *  min(Delx, Dely, Delz)
    doubleprecision,parameter                   :: Delt1 = CFL *  min(Delx, Dely, Delz)
    ...
    doubleprecision,parameter                   :: Delt_loc = 0.1d0
    ...
    doubleprecision,parameter :: alpha_psi = 1.d0
    doubleprecision,parameter :: alpha_phi = 1.d0
