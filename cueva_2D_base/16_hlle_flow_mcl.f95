...
!!$      ! we made avarage over cuadri-vectors ux = 0.5 (Wl vl + Wr vl)
...
!=======================================================================================================================      
   ...
   Cs_x_R      = sqrt(gamma * p_x_R / (enthpy_x_R))
   Cs_x_L      = sqrt(gamma * p_x_L / (enthpy_x_L))

   V2_x_R      =  Vx_x_R**2 +  Vy_x_R**2 + Vz_x_R**2
   V2_x_L      =  Vx_x_L**2 +  Vy_x_L**2 + Vz_x_L**2
...
!/////////////////////// HLL EM Variables  X DIRECTION ////////////////////////////////////
    ...
    q_x_hll   = ( sx2 * qint_x_R   - sx1 * qint_x_L   + Jxint_L   - Jxint_R   ) / (sx2 -sx1)

    !     hll  value of mass conservation 

    DD_x_hll  = ( sx2 * DDint_x_R  - sx1 * DDint_x_L  + FDxint_L  - FDxint_R  ) / (sx2 -sx1)

    !     hll  value of energy conservation 

    tau_x_hll = ( sx2 * tauint_x_R - sx1 * tauint_x_L + Ftauxint_L - Ftauxint_R ) / (sx2 -sx1)

    !    hll  value of momentum S

    Sx_x_hll  = ( sx2 * Sxint_x_R  - sx1 * Sxint_x_L  + FSxxint_L  - FSxxint_R  ) / (sx2 -sx1)
    Sy_x_hll  = ( sx2 * Syint_x_R  - sx1 * Syint_x_L  + FSxyint_L  - FSxyint_R  ) / (sx2 -sx1)
    Sz_x_hll  = ( sx2 * Szint_x_R  - sx1 * Szint_x_L  + FSxzint_L  - FSxzint_R  ) / (sx2 -sx1)
...
!_________________________________________________________________________
       ...
       q_x_hll_flux   = (sx2 * Jxint_L  - sx1 *  Jxint_R + sx1 * sx2 * ( qint_x_R - qint_x_L ) ) / (sx2 - sx1)

! Conserved Mass HLL flux
!_________________________________________________________________________


       DD_x_hll_flux  = (sx2 * FDxint_L  - sx1 * FDxint_R + sx1 * sx2 * ( DDint_x_R - DDint_x_L ) ) / (sx2 - sx1)

! Conserved Energy HLL flux
!_________________________________________________________________________


       tau_x_hll_flux = (sx2 * Ftauxint_L - sx1 * Ftauxint_R + sx1 * sx2 * ( tauint_x_R - tauint_x_L  ) ) / (sx2 - sx1)
...
!_________________________________________________________________________
       ...
       Sx_x_hll_flux  = (sx2 * FSxxint_L - sx1 * FSxxint_R + sx1 * sx2 * (Sxint_x_R - Sxint_x_L ) ) / (sx2 - sx1)
       Sy_x_hll_flux  = (sx2 * FSxyint_L - sx1 * FSxyint_R + sx1 * sx2 * (Syint_x_R - Syint_x_L ) ) / (sx2 - sx1)
       Sz_x_hll_flux  = (sx2 * FSxzint_L - sx1 * FSxzint_R + sx1 * sx2 * (Szint_x_R - Szint_x_L ) ) / (sx2 - sx1)
...
!=======================================================================================================================      
   ...
   Cs_y_R      = sqrt(gamma * p_y_R / (enthpy_y_R))
   Cs_y_L      = sqrt(gamma * p_y_L / (enthpy_y_L))

   V2_y_R      =  Vx_y_R**2 +  Vy_y_R**2 + Vz_y_R**2
   V2_y_L      =  Vx_y_L**2 +  Vy_y_L**2 + Vz_y_L**2
...
!!$   end if
   ...
   if ( isnan(sy1) .or. isnan(sy2)) then
     ...
     print*, " "
      ...
      print*, "NaN value for characteristic velocities sy1 and s1. HLLC soubroutine"
      print*, " "
      stop
...
! we difine de characteristic velocitis in diferent form from notes as -----> sy2 = lambda_r  and sy1 = lambda_l


    !     hll  value of charge density 

    q_y_hll   = ( sy2 * qint_y_R   - sy1 * qint_y_L   + Jyint_L   - Jyint_R   ) / (sy2 -sy1)

    !     hll  value of mass conservation 

    DD_y_hll  = ( sy2 * DDint_y_R  - sy1 * DDint_y_L  + FDyint_L  - FDyint_R  ) / (sy2 -sy1)

    !     hll  value of energy conservation 

    tau_y_hll = ( sy2 * tauint_y_R - sy1 * tauint_y_L + Ftauyint_L - Ftauyint_R ) / (sy2 -sy1)

    !    hll  value of momentum S

    Sy_y_hll  = ( sy2 * Syint_y_R  - sy1 * Syint_y_L  + FSyyint_L  - FSyyint_R  ) / (sy2 -sy1)
    Sz_y_hll  = ( sy2 * Szint_y_R  - sy1 * Szint_y_L  + FSyzint_L  - FSyzint_R  ) / (sy2 -sy1)
    Sx_y_hll  = ( sy2 * Sxint_y_R  - sy1 * Sxint_y_L  + FSyxint_L  - FSyxint_R  ) / (sy2 -sy1)
...
!_________________________________________________________________________
       ...
       q_y_hll_flux   = (sy2 * Jyint_L  - sy1 * Jyint_R + sy1 * sy2 * ( qint_y_R - qint_y_L ) ) / (sy2 -sy1) 

! Conserved Mass HLL flux
!_________________________________________________________________________


       DD_y_hll_flux  = (sy2 * FDyint_L - sy1 * FDyint_R + sy1 * sy2 * ( DDint_y_R - DDint_y_L ) ) / (sy2 -sy1) 

! Conserved Energy HLL flux
!_________________________________________________________________________


       tau_y_hll_flux = (sy2 * Ftauyint_L - sy1 * Ftauyint_R + sy1 * sy2 * ( tauint_y_R - tauint_y_L  ) ) / (sy2 -sy1) 
...
!_________________________________________________________________________
       ...
       Sy_y_hll_flux  = (sy2 * FSyyint_L - sy1 * FSyyint_R + sy1 * sy2 * (Syint_y_R - Syint_y_L ) ) / (sy2 -sy1) 
       Sz_y_hll_flux  = (sy2 * FSyzint_L - sy1 * FSyzint_R + sy1 * sy2 * (Szint_y_R - Szint_y_L ) ) / (sy2 -sy1) 
       Sx_y_hll_flux  = (sy2 * FSyxint_L - sy1 * FSyxint_R + sy1 * sy2 * (Sxint_y_R - Sxint_y_L ) ) / (sy2 -sy1) 
...
! -------------------------------------- EGLM-------------------------------------- 
    ...
    else if (Vxstr .lt. 0.d0 .and. 0.d0 .le. sx2) then
...
! -------------------------------------- EGLM-------------------------------------- 
    ...
    else if (Vystr .lt. 0.d0 .and. 0.d0 .le. sy2) then
