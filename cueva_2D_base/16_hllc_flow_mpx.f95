         ...
         do i=-6,imax+6
            ...
            if (REC_PRIM == 0) then
               ...
               if (REC_4VECTOR == 0) then
                  ...
                  W(i,1,1) = 1.d0 / sqrt( 1.d0 -      &
                                       Vx(i,1,1)**2 + &
                                       Vy(i,1,1)**2 + &
                                       Vz(i,1,1)**2 )

                  u(i,21,1) = W(i,1,1) * Vx (i,1,1)
                  u(i,22,1) = W(i,1,1) * Vy (i,1,1)
                  u(i,23,1) = W(i,1,1) * Vz (i,1,1)
...
!!$               
               ...
               u(i,25,1) = rhoint(i,1,1,l)
                  ...
                  Wint(i,1,1,l) = 1.d0 / sqrt( 1.d0 -      &
                                       Vxint(i,1,1,l)**2 + &
                                       Vyint(i,1,1,l)**2 + &
                                       Vzint(i,1,1,l)**2 )

                  u(i,21,1) = Wint(i,1,1,l) * Vxint (i,1,1,l)
                  u(i,22,1) = Wint(i,1,1,l) * Vyint (i,1,1,l)
                  u(i,23,1) = Wint(i,1,1,l) * Vzint (i,1,1,l)
...
!//////////////////////////// LORENTZ FACTOR ///////////////////////////////
     ...
     if (REC_4VECTOR == 0) then
        ...
        V2mp5_x_L = (Vx_L**2+Vy_L**2+Vz_L**2)
        V2mp5_x_R = (Vx_R**2+Vy_R**2+Vz_R**2)
...
!               print*, "super luminal left velocity in mp5 soubroutine, V2_x =", V2mp5_x_L, "i=", i, "j=", j
        ...
        W_L       = 1.d0/sqrt(1.d0 -  V2mp5_x_L )
        W_R       = 1.d0/sqrt(1.d0 -  V2mp5_x_R )

        
     else if (REC_4VECTOR == 1) then

             W_L = sqrt( 1.d0 + Ux_L**2 &
                              + Uy_L**2 &
                              + Uz_L**2 )
...
! 1) First calculate the continuos conserved variables in start region (q, E, B) as its avarage HLL values
   ...
   p_elec      =  0.5d0 * ( p_L + p_R )
   eps1        =  1.e-6
   
   !Drift velocity approximation Vxstr
    
   if (VDRIFT == 1 .and. p_elec .le. (gamma - 1.d0) * E2B2_str_x) then !

      Vxstr    =  ErotB_str_x / E2B2_str
      ...
      if ( abs(Vxstr) .le. 1.d-9 ) then
         ...
         print*, "superluminal velocity ( -1 > Vxstr > 1) in  DRIFT velocity HLLC Solver"
         print*, "(Vxstr, h, i, j, l) --->", Vxstr, h, i, j, l

         Vxstr= max(-1.d0 + eps1 , min( 1.d0 - eps1, Vxstr ))

      end if

   else !else DRIFT velocity approximation (Normal precedure for less magnetized plasmas) Vxstr
...
!!$end if
   ...
   psistr_R =  ( s2 * psi_hll   - Vxstr * Ex_hll_flux) / (s2 - Vxstr) 
   psistr_L =  (-s1 * psi_hll   + Vxstr * Ex_hll_flux) / (Vxstr - s1) 
   
   phistr_R = ( s2 * phi_hll    - Vxstr * Bx_hll_flux) / (s2 - Vxstr) 
   phistr_L = (-s1 * phi_hll    + Vxstr * Bx_hll_flux) / (Vxstr - s1)
...
!   EM FLUXES       
    ...
    psistr_flux_R =  psi_hll_flux  + s2 * (Vxstr - s1) * (psistr_R - psistr_L) / (s2 -s1) !Exint_R  + s2 * ( psistr_R - psiint_R) ! Exstr_R !
    psistr_flux_L =  psi_hll_flux  - s1 * (s2 - Vxstr) * (psistr_R - psistr_L) / (s2 -s1) !Exint_L  + s1 * ( psistr_L - psiint_L) ! Exstr_L !
    
    phistr_flux_R =  phi_hll_flux  + s2 * (Vxstr - s1) * (phistr_R - phistr_L) / (s2 -s1) !Bxint_R  + s2 * ( phistr_R - phiint_R) ! Bxstr_R !
    phistr_flux_L =  phi_hll_flux  - s1 * (s2 - Vxstr) * (phistr_R - phistr_L) / (s2 -s1) !Bxint_L  + s1 * ( phistr_L - phiint_L) ! Bxstr_L !
...
!!$    Ftauxstr_R    = Ftauxint_R + s2 * (taustr_R - tauint_R) ! Sxstr_R !
!!$    Ftauxstr_L    = Ftauxint_L + s1 * (taustr_L - tauint_L) ! Sxstr_L !
...
!_________________________________________________________________________
       ...
       if (source_rec_mpx == 1) then
          ...
          Ezastsour(i,j,1,l) = 0.5d0 * (Eysource_L + Eysource_R)
...
! -------------------------------------- EGLM-------------------------------------- 
       ...
       print*, "Revise Vstar velocity it could be imaginary subroutine hllc_flow "
       stop
...
!$OMP & PRIVATE(dim_x,dim_y,rho__,not__,V2mp5,V2mp5_x,V2mp5_y,sigma_L,sigma_R,test_rho_L,test_rho_R, imp),                        &
...
!!$                         Edotv_mp5_x_R(i,j) * Vz_mp5_x_R(i,j)  )+  qint_mp5_x_R(i,j) * Vz_mp5_x_R(i,j) 
            ...
            if (rho_mp5_y_L(i,j) .lt. 0.d0) then

               print*, "Negative density mp5 test_rho_L=", rho_mp5_y_L(i,j), "i=", i, "j=", j

               rho_mp5_y_L(i,j)    = rho(i,j,1) * 1.d-1 !max( abs(rho_mp5_y_L(i,j)) * 1.d-1, rho_mp5_y_L(i,j) )
!               rho_mp5_y_R(i,j)    = rho(i,j,1) * 1.d-1

               
            else if (rho_mp5_y_R(i,j) .lt. 0.d0) then

               print*, "Negative density mp5 test_rho_R=", rho_mp5_y_R(i,j), "i=", i, "j=", j

               rho_mp5_y_R(i,j)    = rho(i,j,1) * 1.d-1 != max( abs(rho_mp5_y_R(i,j)) * 1.d-1, rho_mp5_y_R(i,j) )
!               rho_mp5_y_L(i,j)    = rho(i,j,1) * 1.d-1
               
            else if (rho_mp5_x_L(i,j) .lt. 0.d0) then

               print*, "Negative density mp5 test_rho_R=", rho_mp5_x_L(i,j), "i=", i, "j=", j

               rho_mp5_x_L(i,j)    = rho(i,j,1) * 1.d-1 != max( abs(rho_mp5_x_L(i,j)) * 1.d-1, rho_mp5_x_L(i,j) )
!               rho_mp5_x_R(i,j)    = rho(i,j,1) * 1.d-1 
               
            else if (rho_mp5_x_R(i,j) .lt. 0.d0) then

               print*, "Negative density mp5 test_rho_R=", rho_mp5_x_R(i,j), "i=", i, "j=", j

               rho_mp5_x_R(i,j)    = rho(i,j,1) * 1.d-1 != max( abs(rho_mp5_x_R(i,j)) * 1.d-1, rho_mp5_x_R(i,j) )  
!               rho_mp5_x_L(i,j)    = rho(i,j,1) * 1.d-1 
            end if
...
! we difine de characteristic velocitis in diferent form from notes as -----> s2 = lambda_r  and s1 = lambda_l
    ...
    q_x_hll   = ( s2 * qint_mp5_x_R(i,j)   - s1 * qint_mp5_x_L(i,j)   &
         + Jxint_mp5_L(i,j)   - Jxint_mp5_R(i,j)   ) / (s2 -s1)

    !     hll  value of mass conservation 

    DD_x_hll  = ( s2 * DDint_mp5_x_R(i,j)  - s1 * DDint_mp5_x_L(i,j)  &
         + FDxint_mp5_L(i,j)  - FDxint_mp5_R(i,j)  ) / (s2 -s1)

    !     hll  value of energy conservation 

    tau_x_hll = ( s2 * tauint_mp5_x_R(i,j) - s1 * tauint_mp5_x_L(i,j) &
         + Ftauxint_mp5_L(i,j) - Ftauxint_mp5_R(i,j) ) / (s2 -s1)

    !    hll  value of momentum S

    Sx_x_hll  = ( s2 * Sxint_mp5_x_R(i,j)  - s1 * Sxint_mp5_x_L(i,j)  &
         + FSxxint_mp5_L(i,j)  - FSxxint_mp5_R(i,j)  ) / (s2 -s1)
    Sy_x_hll  = ( s2 * Syint_mp5_x_R(i,j)  - s1 * Syint_mp5_x_L(i,j)  &
         + FSxyint_mp5_L(i,j)  - FSxyint_mp5_R(i,j)  ) / (s2 -s1)
    Sz_x_hll  = ( s2 * Szint_mp5_x_R(i,j)  - s1 * Szint_mp5_x_L(i,j)  &
         + FSxzint_mp5_L(i,j)  - FSxzint_mp5_R(i,j)  ) / (s2 -s1)
...
!_________________________________________________________________________
       ...
       q_x_hll_flux   =   0.5d0 * (Jxint_mp5_L(i,j)  + Jxint_mp5_R(i,j) &
            - ( qint_mp5_x_R(i,j) - qint_mp5_x_L(i,j) ) )

! Conserved Mass HLL flux
!_________________________________________________________________________


       DD_x_hll_flux  =   0.5d0 * (FDxint_mp5_L(i,j)  + FDxint_mp5_R(i,j) &
            - ( DDint_mp5_x_R(i,j) - DDint_mp5_x_L(i,j) ) )

! Conserved Energy HLL flux
!_________________________________________________________________________


       tau_x_hll_flux =   0.5d0 * (Ftauxint_mp5_L(i,j) + Ftauxint_mp5_R(i,j) &
            - ( tauint_mp5_x_R(i,j) - tauint_mp5_x_L(i,j)  ) )
...
!_________________________________________________________________________
       ...
       Sx_x_hll_flux  =   0.5d0 * (FSxxint_mp5_L(i,j) + FSxxint_mp5_R(i,j) &
            - (Sxint_mp5_x_R(i,j) - Sxint_mp5_x_L(i,j) ) ) 
       Sy_x_hll_flux  =   0.5d0 * (FSxyint_mp5_L(i,j) + FSxyint_mp5_R(i,j) &
            - (Syint_mp5_x_R(i,j) - Syint_mp5_x_L(i,j) ) ) 
       Sz_x_hll_flux  =   0.5d0 * (FSxzint_mp5_L(i,j) + FSxzint_mp5_R(i,j) &
            - (Szint_mp5_x_R(i,j) - Szint_mp5_x_L(i,j) ) )
...
! we difine de characteristic velocitis in diferent form from notes as -----> s2 = lambda_r  and s1 = lambda_l
    ...
    Ez_y_hll  = ( s2 * Ezint_mp5_y_R(i,j)  - s1 * Ezint_mp5_y_L(i,j)  + Bxint_mp5_y_L(i,j)   - Bxint_mp5_y_R(i,j)   ) / (s2 -s1)
    Ex_y_hll  = ( s2 * Exint_mp5_y_R(i,j)  - s1 * Exint_mp5_y_L(i,j)  + Bzint_mp5_y_L_m(i,j) - Bzint_mp5_y_R_m(i,j) ) / (s2 -s1)  !ojo minus sign ¡¡¡


    !     hll  values of magnetic field

    By_y_hll  = ( s2 * Byint_mp5_y_R(i,j)  - s1 * Byint_mp5_y_L(i,j)  + phiint_mp5_y_L(i,j)  - phiint_mp5_y_R(i,j)  ) / (s2 -s1)
    Bz_y_hll  = ( s2 * Bzint_mp5_y_R(i,j)  - s1 * Bzint_mp5_y_L(i,j)  + Exint_mp5_y_L_m(i,j) - Exint_mp5_y_R_m(i,j) ) / (s2 -s1)  !ojo minus sign ¡¡¡
    Bx_y_hll  = ( s2 * Bxint_mp5_y_R(i,j)  - s1 * Bxint_mp5_y_L(i,j)  + Ezint_mp5_y_L(i,j)   - Ezint_mp5_y_R(i,j)   ) / (s2 -s1)


    !     hll  value of charge density 

    q_y_hll   = ( s2 * qint_mp5_y_R(i,j)   - s1 * qint_mp5_y_L(i,j)   + Jyint_mp5_L(i,j)   - Jyint_mp5_R(i,j)   ) / (s2 -s1)

    !     hll  value of mass conservation 

    DD_y_hll  = ( s2 * DDint_mp5_y_R(i,j)  - s1 * DDint_mp5_y_L(i,j)  + FDyint_mp5_L(i,j)  - FDyint_mp5_R(i,j)  ) / (s2 -s1)

    !     hll  value of energy conservation 

    tau_y_hll = ( s2 * tauint_mp5_y_R(i,j) - s1 * tauint_mp5_y_L(i,j) + Ftauyint_mp5_L(i,j) - Ftauyint_mp5_R(i,j) ) / (s2 -s1)

    !    hll  value of momentum S

    Sy_y_hll  = ( s2 * Syint_mp5_y_R(i,j)  - s1 * Syint_mp5_y_L(i,j)  + FSyyint_mp5_L(i,j)  - FSyyint_mp5_R(i,j)  ) / (s2 -s1)
    Sz_y_hll  = ( s2 * Szint_mp5_y_R(i,j)  - s1 * Szint_mp5_y_L(i,j)  + FSyzint_mp5_L(i,j)  - FSyzint_mp5_R(i,j)  ) / (s2 -s1)
    Sx_y_hll  = ( s2 * Sxint_mp5_y_R(i,j)  - s1 * Sxint_mp5_y_L(i,j)  + FSyxint_mp5_L(i,j)  - FSyxint_mp5_R(i,j)  ) / (s2 -s1)
...
!_________________________________________________________________________
      ...
      psi_y_hll_flux  =   0.5d0 * (Eyint_mp5_y_L(i,j) + Eyint_mp5_y_R(i,j)- ( psiint_mp5_y_R(i,j) - psiint_mp5_y_L(i,j) ) )  
...
!_________________________________________________________________________
       ...
       q_y_hll_flux   =   0.5d0 * (Jyint_mp5_L(i,j)  + Jyint_mp5_R(i,j) &
            - ( qint_mp5_y_R(i,j) - qint_mp5_y_L(i,j) ) )

! Conserved Mass HLL flux
!_________________________________________________________________________


       DD_y_hll_flux  =   0.5d0 * (FDyint_mp5_L(i,j)  + FDyint_mp5_R(i,j) &
            - ( DDint_mp5_y_R(i,j) - DDint_mp5_y_L(i,j) ) )

! Conserved Energy HLL flux
!_________________________________________________________________________


       tau_y_hll_flux =   0.5d0 * (Ftauyint_mp5_L(i,j) + Ftauyint_mp5_R(i,j) &
            - ( tauint_mp5_y_R(i,j) - tauint_mp5_y_L(i,j)  ) )
...
!_________________________________________________________________________
       ...
       Sy_y_hll_flux  =   0.5d0 * (FSyyint_mp5_L(i,j) + FSyyint_mp5_R(i,j) &
            - (Syint_mp5_y_R(i,j) - Syint_mp5_y_L(i,j) ) ) 
       Sz_y_hll_flux  =   0.5d0 * (FSyzint_mp5_L(i,j) + FSyzint_mp5_R(i,j) &
            - (Szint_mp5_y_R(i,j) - Szint_mp5_y_L(i,j) ) ) 
       Sx_y_hll_flux  =   0.5d0 * (FSyxint_mp5_L(i,j) + FSyxint_mp5_R(i,j) &
            - (Sxint_mp5_y_R(i,j) - Sxint_mp5_y_L(i,j) ) )
...
! 1) First calculate the continuos conserved variables in start region (q, E, B) as its avarage HLL values
   ...
   p_elec      =  0.5d0 * ( p_mp5_x_L(i,j) + p_mp5_x_R(i,j) )
   ...
   if (VDRIFT == 1 .and. p_elec .le. (gamma - 1.d0) * E2B2_str_x) then !
      ...
      Vxstr    =  ErotB_str_x_x_L / E2B2_str_x
...
!!$   in the physical range [-1,1] usign a Newton-Rapson iterative method.
  ...
  end if ! end conditional DRIFT velocity Vxstr
...
!!$   end if   
   ...
   p_elec      =  0.5d0 * ( p_mp5_y_L(i,j) + p_mp5_y_R(i,j) )

   eps1        = 1.e-6

   !Drift velocity approximation Vystr
    
   if (VDRIFT == 1 .and. p_elec .le. (gamma - 1.d0) * E2B2_str_y) then !

      Vystr    =  ErotB_str_y_y_L / E2B2_str_y
      ...
      if ( abs(Vystr) .le. 1.d-9 ) then
         ...
         print*, "superluminal velocity ( -1 > Vystr > 1) in  DRIFT velocity HLLC Solver"
         print*, "(Vystr, h, i, j, l) --->", Vystr, h, i, j, l

         Vystr= max(-1.d0 + eps1 , min( 1.d0 - eps1, Vystr ))

      end if

   else !else DRIFT velocity approximation (Normal precedure for less magnetized plasmas) Vystr
...
!!$   in the physical range [-1,1] usign a Newton-Rapson iterative method.
   ...
   end if   ! end conditional DRIFT velocity Vystr
...
!!$              / (Vxstr - s1) 
   ...
   Systr_x_R  = ( s2 * Sy_x_hll - Sy_x_hll_flux - ErotB_str_y_x_R * Vxstr &
              - Exstr_x_R * Eystr_x_R - Bxstr_x_R * Bystr_x_R )           &
              / ( s2 - Vxstr)
   Systr_x_L  = (-s1 * Sy_x_hll + Sy_x_hll_flux + ErotB_str_y_x_R * Vxstr &
              + Exstr_x_L * Eystr_x_L + Bxstr_x_L * Bystr_x_L )           &
              / ( Vxstr - s1)

   Szstr_x_R  = ( s2 * Sz_x_hll - Sz_x_hll_flux - ErotB_str_z_x_R * Vxstr &
              - Exstr_x_R * Ezstr_x_R - Bxstr_x_R * Bzstr_x_R )           &
              / ( s2 - Vxstr)
   Szstr_x_L  = (-s1 * Sz_x_hll + Sz_x_hll_flux + ErotB_str_z_x_R * Vxstr &
              + Exstr_x_L * Ezstr_x_L + Bxstr_x_L * Bzstr_x_L )           &
              / ( Vxstr - s1)
...
!!$              / (Vystr - s1) 
   ...
   Szstr_y_R  = ( s2 * Sz_y_hll - Sz_y_hll_flux - ErotB_str_z_y_R * Vystr &
              - Eystr_y_R * Ezstr_y_R - Bystr_y_R * Bzstr_y_R )           &
              / ( s2 - Vystr)
   Szstr_y_L  = (-s1 * Sz_y_hll + Sz_y_hll_flux + ErotB_str_z_y_R * Vystr &
              + Eystr_y_L * Ezstr_y_L + Bystr_y_L * Bzstr_y_L )           &
              / (Vystr - s1)

   Sxstr_y_R  = ( s2 * Sx_y_hll - Sx_y_hll_flux - ErotB_str_x_y_R * Vystr &
              - Eystr_y_R * Exstr_y_R - Bystr_y_R * Bxstr_y_R )           &
              / ( s2 - Vystr)
   Sxstr_y_L  = (-s1 * Sx_y_hll + Sx_y_hll_flux + ErotB_str_x_y_R * Vystr &
              + Eystr_y_L * Exstr_y_L + Bystr_y_L * Bxstr_y_L )           &
              / (Vystr - s1)
...
! these values must be canceled when we made diference of fluxes --->
 ...
 ! EM FLUXES
    ...
    psistr_x_flux_R =  psi_x_hll_flux + (s2 * (Vxstr - s1) &
         * (psistr_x_R - psistr_x_L) )/(s2 -s1) 
    psistr_x_flux_L =  psi_x_hll_flux - (s1 * (s2 - Vxstr) &
         * (psistr_x_R - psistr_x_L) )/(s2 -s1) 
    
    phistr_x_flux_R =  phi_x_hll_flux + (s2 * (Vxstr - s1) &
         * (phistr_x_R - phistr_x_L) )/(s2 -s1) 
    phistr_x_flux_L =  phi_x_hll_flux - (s1 * (s2 - Vxstr) &
         * (phistr_x_R - phistr_x_L) )/(s2 -s1) 
    ...
    FDxstr_R = DD_x_hll_flux  + s2 * (Vxstr - s1) &
         * (DDstr_x_R - DDstr_x_L) / (s2 -s1)
    FDxstr_L = DD_x_hll_flux  - s1 * (s2 - Vxstr) &
         * (DDstr_x_R - DDstr_x_L) / (s2 -s1)
...
! these values must be canceled when we made diference of fluxes --->
    ...
    Ftauxstr_R = tau_x_hll_flux  + s2 * (Vxstr - s1) &
         * (taustr_x_R - taustr_x_L) / (s2 -s1)
    Ftauxstr_L = tau_x_hll_flux  - s1 * (s2 - Vxstr) &
         * (taustr_x_R - taustr_x_L) / (s2 -s1)
...
! these values must be canceled when we made diference of fluxes --->
    ...
    FSxxstr_R =  Sx_x_hll_flux  + s2 * (Vxstr - s1) &
         * (Sxstr_x_R - Sxstr_x_L) / (s2 -s1)
    FSxxstr_L =  Sx_x_hll_flux  - s1 * (s2 - Vxstr) &
         * (Sxstr_x_R - Sxstr_x_L) / (s2 -s1)

    FSxystr_R =  Sy_x_hll_flux  + s2 * (Vxstr - s1) &
         * (Systr_x_R - Systr_x_L) / (s2 -s1)
    FSxystr_L =  Sy_x_hll_flux  - s1 * (s2 - Vxstr) &
         * (Systr_x_R - Systr_x_L) / (s2 -s1)

    FSxzstr_R =  Sz_x_hll_flux  + s2 * (Vxstr - s1) &
         * (Szstr_x_R - Szstr_x_L) / (s2 -s1)
    FSxzstr_L =  Sz_x_hll_flux  - s1 * (s2 - Vxstr) &
         * (Szstr_x_R - Szstr_x_L) / (s2 -s1)
...
!     Current density expression J = sigma * W * (E + V X B - (E.V)V) + qV
 ...
 ! EM FLUXES
    ...
    psistr_y_flux_R =  psi_y_hll_flux + (s2 * (Vystr - s1) &
         * (psistr_y_R - psistr_y_L) )/(s2 -s1) 
    psistr_y_flux_L =  psi_y_hll_flux - (s1 * (s2 - Vystr) &
         * (psistr_y_R - psistr_y_L) )/(s2 -s1) 
    
    phistr_y_flux_R =  phi_y_hll_flux + (s2 * (Vystr - s1) &
         * (phistr_y_R - phistr_y_L) )/(s2 -s1) 
    phistr_y_flux_L =  phi_y_hll_flux - (s1 * (s2 - Vystr) &
         * (phistr_y_R - phistr_y_L) )/(s2 -s1) 
    ...
    FDystr_R = DD_y_hll_flux  + s2 * (Vystr - s1) &
         * (DDstr_y_R - DDstr_y_L) / (s2 -s1)
    FDystr_L = DD_y_hll_flux  - s1 * (s2 - Vystr) &
         * (DDstr_y_R - DDstr_y_L) / (s2 -s1)
    ...
    Ftauystr_R = tau_y_hll_flux  + s2 * (Vystr - s1) &
         * (taustr_y_R - taustr_y_L) / (s2 -s1)
    Ftauystr_L = tau_y_hll_flux  - s1 * (s2 - Vystr) &
         * (taustr_y_R - taustr_y_L) / (s2 -s1)
    ...
    FSyxstr_R =  Sx_y_hll_flux  + s2 * (Vystr - s1) &
         * (Sxstr_y_R - Sxstr_y_L) / (s2 -s1)
    FSyxstr_L =  Sx_y_hll_flux  - s1 * (s2 - Vystr) &
         * (Sxstr_y_R - Sxstr_y_L) / (s2 -s1)

    FSyystr_R =  Sy_y_hll_flux  + s2 * (Vystr - s1) &
         * (Systr_y_R - Systr_y_L) / (s2 -s1)
    FSyystr_L =  Sy_y_hll_flux  - s1 * (s2 - Vystr) &
         * (Systr_y_R - Systr_y_L) / (s2 -s1)

    FSyzstr_R =  Sz_y_hll_flux  + s2 * (Vystr - s1) &
         * (Szstr_y_R - Szstr_y_L) / (s2 -s1)
    FSyzstr_L =  Sz_y_hll_flux  - s1 * (s2 - Vystr) &
         * (Szstr_y_R - Szstr_y_L) / (s2 -s1)
